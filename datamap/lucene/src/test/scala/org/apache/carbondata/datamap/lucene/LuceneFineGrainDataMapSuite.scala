  ...
  test("test lucene fine grain data map") {
    ...
    sql(
      s"""
         ...
         | USING '${classOf[LuceneFineGrainDataMapFactory].getName}'
      """.stripMargin)
    ...
    sql("select * from datamap_test where match('name:n10')").show
    sql("select * from datamap_test where match('name:n10*')").show
    sql("select * from datamap_test where match('city:c020')").show
