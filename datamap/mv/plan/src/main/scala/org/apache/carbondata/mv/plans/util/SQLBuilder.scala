          ...
          plan.transformExpressions {
            case ref: Attribute =>
              ...
              if (i > -1) { 
                ...
                if (aliasMap.nonEmpty && aliasMap(i).nonEmpty) AttributeReference(ref.name, ref.dataType)(exprId = ref.exprId, qualifier = Option(aliasMap(i)))
                else ref
              }
              else {
          ...
          s.children.zipWithIndex.filterNot { _._1.isInstanceOf[modular.LeafNode] }.foreach {
            case (child: ModularPlan, index) if (!s.aliasMap.contains(index)) => {
              ...
              val windowAttributeSet = if (child.isInstanceOf[Select]) {
                val windowExprs = child.asInstanceOf[Select].windowSpec.map{case Seq(expr) => expr.asInstanceOf[Seq[NamedExpression]]}.foldLeft(Seq.empty.asInstanceOf[Seq[NamedExpression]])(_ ++ _)
