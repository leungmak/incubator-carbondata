...
public class DefaultEncodingStrategy extends EncodingStrategy {
  ...
  protected DataType fitDataType(long value) {
  ...
  ColumnPageCodec newCodecForIntegerType(ColumnPageStatsVO stats) {
    ...
    if (adaptiveDataType.getSizeInBytes() <= deltaDataType.getSizeInBytes()) {
      ...
      return AdaptiveIntegerCodec.newInstance(stats.getDataType(), adaptiveDataType, stats);
    } else {
      // choose delta adaptive encoding
      return DeltaIntegerCodec.newInstance(stats.getDataType(), deltaDataType, stats);
    }
  }
  ...
  ColumnPageCodec newCodecForFloatingType(ColumnPageStatsVO stats) {
    ...
    if (decimal == 0) {
      ...
      if (adaptiveDataType.getSizeInBytes() <= deltaDataType.getSizeInBytes()) {
        // choose adaptive encoding
        return AdaptiveIntegerCodec.newInstance(srcDataType, adaptiveDataType, stats);
      } else {
        // choose delta adaptive encoding
        return DeltaIntegerCodec.newInstance(srcDataType, deltaDataType, stats);
      }
    } else {
      // double
      ...
      if (upscaleAdaptiveDataType.getSizeInBytes() <= upscaleDiffDataType.getSizeInBytes()) {
        return UpscaleFloatingCodec.newInstance(srcDataType, upscaleAdaptiveDataType, stats);
      } else {
        return UpscaleDeltaFloatingCodec.newInstance(srcDataType, upscaleDiffDataType, stats);
      }
    }
  }
  ...
  ColumnPageCodec newCodecForDecimalType(ColumnPageStatsVO stats) {
    return VarLengthTypeCodec.newInstance(stats.getDataType());
  }

  @Override
  ColumnPageCodec newCodecForStringType(ColumnPageStatsVO stats) {
    return VarLengthTypeCodec.newInstance(stats.getDataType());
  }
}
