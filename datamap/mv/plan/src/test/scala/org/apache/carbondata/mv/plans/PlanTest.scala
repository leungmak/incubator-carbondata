...
import org.apache.carbondata.mv.plans.modular.{OneRowTable, Select}
...
abstract class PlanTest extends SparkFunSuite with PredicateHelper {
   ...
   */
  protected def normalizeExprIds(plan: ModularPlan) = {
...
//  }
  ...
  protected def compareExpressions(e1: Seq[Expression], e2: Seq[Expression]): Unit = {
    comparePlans(Select(Nil,Nil,e1,Map.empty,Nil,Seq(OneRowTable),NoFlags,Seq.empty,Seq.empty), modular
