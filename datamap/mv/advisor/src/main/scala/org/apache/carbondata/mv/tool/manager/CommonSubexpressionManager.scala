...
//        val combinedOutputList = (for (exprs <- combinedOutputGroups.values) yield (exprs.head match { case a: Alias if a.child.isInstanceOf[Substring] => a.copy(child = substringCovering(exprs.map(_.asInstanceOf[Alias].child.asInstanceOf[Substring])))(a.exprId,a.qualifier,a.explicitMetadata,a.isGenerated); case o => o.asInstanceOf[NamedExpression]})).toSeq
        val combinedOutputGroups = (outputListR ++ outputListM ++ predicateListR ++ predicateListM ++ combinedResult._3).groupBy{case a: Alias => a.child.semanticHash(); case e => e.semanticHash()}

        // TODO: how to handle Substring, which can not be converted to NamedExpression
        val combinedOutputList = (for (exprs <- combinedOutputGroups.values) yield (if (exprs.head.isInstanceOf[NamedExpression]) exprs.head.asInstanceOf[NamedExpression] else Alias(exprs.head, exprs.head.toString)())).toSeq

        val combinedInputGroups = (inputListR ++ inputListM ++ combinedResult._3).groupBy(_.semanticHash())
        val combinedInputList = (for (attrs <- combinedInputGroups.values) yield (attrs.head.asInstanceOf[NamedExpression])).toSeq
...
//        val combinedPredicateList = (for (exprs <- combinedPredicateGroups.values) yield (if (exprs.size == 1) exprs.head else substringCovering(exprs.asInstanceOf[Seq[Substring]]))).toSeq
        val combinedPredicateGroups = (predicateListR ++ predicateListM ++ combinedResult._3).groupBy{_.semanticHash}
        val combinedPredicateList = (for (exprs <- combinedPredicateGroups.values) yield (exprs.head)).toSeq
      ...
      case (r @ modular.Select(outputListR, inputListR, predicateListR, aliasMapR, joinEdgesR, childrenR, _, _, _), m @ modular.Select(outputListM, inputListM, predicateListM, aliasMapM, joinEdgesM, childrenM, _, _, _)) => {
        ...
        val combinedOutputGroups = (outputListR ++ outputListM).groupBy{case a: Alias => a.child.semanticHash(); case e => e.semanticHash()}
        val combinedOutputList = (for (exprs <- combinedOutputGroups.values) yield (exprs.head)).toSeq
        ...
        if (combinedPredicateList.size == r.joinEdges.size && combinedChildren.size == r.children.size) (true, Some(modular.Select(combinedOutputList, combinedInputList, combinedPredicateList, aliasMap, r.joinEdges, combinedChildren, NoFlags, Seq.empty, Seq.empty)), extendedAttributeSet.toSeq)
