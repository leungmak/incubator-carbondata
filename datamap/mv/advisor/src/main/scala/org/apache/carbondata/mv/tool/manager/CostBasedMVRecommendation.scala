...
case class CostBasedMVRecommendation(spark: SparkSession, conf: SQLConf) extends PredicateHelper {
  ...
  lazy val speedUpThreshold = conf.getConfString("spark.mv.recommend.speedup.threshold").toDouble
  lazy val sizeReductionThreshold = conf.getConfString("spark.mv.recommend.rowcount.threshold").toDouble
  lazy val frequencyThrehold = conf.getConfString("spark.mv.recommend.frequency.threshold").toInt
        ...
        computeCandidateWithMaximalBenefit(candidateR, trivialCandidateBufM) match {
          case Some(bestCandidateWithBenefit) =>
            if (bestCandidateWithBenefit._1 > speedUpThreshold) {
              val result = ModularPlanUtility.combinePlans(candidateR._1.plan, bestCandidateWithBenefit._2._1.plan, isFirst)
              isFirst = false
              (result._1, result._2) match {
                case (true, Some(combinedPlan)) =>
                  val candidateR1 = (MVAccessCardinality(combinedPlan, getMVAccessCardinality(combinedPlan)), candidateR._2 + bestCandidateWithBenefit._2._2)
                  if (notEnoughSpeedUp(candidateR1._1)) {
                    ...
                    isCandidate = true
//                    val s = trivialCandidateBufM.length
                    trivialCandidateBufM -= bestCandidateWithBenefit._2
//                    val t = trivialCandidateBufM.length
      ...
      if (isCandidate) candidateSet += candidateR
        ...
        val result = ModularPlanUtility.combinePlans(candidateR._1.plan, candidateR._1.plan, true)       
  ...
  private def computeCandidateWithMaximalBenefit(rSubplan: (MVAccessCardinality, Int), mCandidates: ArrayBuffer[(MVAccessCardinality, Int)]): Option[(Float, (MVAccessCardinality, Int))] = {
    val fNumOfRows = rSubplan._1.plan.asInstanceOf[modular.GroupBy].child.children(0).asInstanceOf[ModularRelation].stats(spark, conf).rowCount
        ...
        for (candidate <- mCandidates) {
          ...
          cNumOfRows match {
            case Some(c) => {
              ...
              val speedupNew = (rSubplan._2 * speedupR + candidate._2 * speedupC) / (rSubplan._2 + candidate._2)
  ...
  def notEnoughSpeedUp(candidate: MVAccessCardinality): Boolean = {
    ...
    val sNumOfRows = candidate.plan.asInstanceOf[modular.GroupBy].child.children(0).asInstanceOf[ModularRelation].stats(spark, conf).rowCount
    (rNumOfRows, sNumOfRows) match {
      case (Some(r), Some(s)) => if (r.toDouble > sizeReductionThreshold * s.toDouble) true else false
      case _ => false
    }
  }
  ...
  private def getMVAccessCardinality(
      input: ModularPlan): Option[BigInt] = input match {
    case g @ modular.GroupBy(_, _, _, _, modular.Select(_, _, _, _, _, children, NoFlags, Nil, Nil), NoFlags, Nil) =>
      val statsList = children.map(_.stats(spark,conf))
      val ndvs = g.predicateList.map { 
        case subexp @ Substring(col: AttributeReference, pos:Literal, len:Literal) if pos.dataType == IntegerType && len.dataType == IntegerType => {
          import scala.math.min
          statsList.find(_.attributeStats.contains(col)).map(_.attributeStats.get(col)).flatten.map(colstats => BigInt((colstats.distinctCount.toDouble * min((len.value.asInstanceOf[Integer].toFloat / colstats.maxLen.toFloat).toDouble, 1.0)).toLong))
        }
        case col: AttributeReference => 
          statsList.find(_.attributeStats.contains(col)).map(_.attributeStats.get(col)).flatten.map(_.distinctCount)
        case other => None }.flatten 
