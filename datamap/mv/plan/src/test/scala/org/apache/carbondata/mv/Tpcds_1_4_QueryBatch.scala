...
object Tpcds_1_4_QueryBatch {
  ...
  val tpcds1_4Queries = Seq(
    ...
    ("qMV2", """
            ...
            """.stripMargin.trim),
    // this query shows a bug in modular plan where qualifier subq not clean up in erasure of sub-query alias in logical plan 
    ("qMV3", """
              |SELECT first(subq.channel), subq.col_name, subq.d_year, subq.d_qoy, subq.i_category, SUM(subq.ss_ext_sales_price) sales_amt
              |FROM
              | (SELECT
              |      'store' as channel, ss_store_sk col_name, d_year, d_qoy, i_category,
              |      ss_ext_sales_price
    ...
    ("qVerify",
        """
         |SELECT gen_subquery_0.`c_last_name`, gen_subquery_0.`c_first_name`, gen_subquery_0.`d_date`, gen_subquery_1.`c_last_name`, gen_subquery_1.`c_first_name`, gen_subquery_1.`d_date` 
         |        FROM
         ...
         |          GROUP BY customer.`c_last_name`, customer.`c_first_name`, date_dim.`d_date`) gen_subquery_1  ON (gen_subquery_0.`c_last_name` <=> gen_subquery_1.`c_last_name`) AND (gen_subquery_0.`c_first_name` <=> gen_subquery_1.`c_first_name`) AND (gen_subquery_0.`d_date` <=> gen_subquery_1.`d_date`)
        """.stripMargin),
      ("qSsMax",
        """
          |select
