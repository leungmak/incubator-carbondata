...
public class BatchSegment implements Segment {
  ...
  private BatchSegment(Table table) {
    this.table = table.getMeta();
    this.opened = false;
    this.taskId = 0;
  }

  public static BatchSegment newInstace(Table table) {
  ...
  private TaskAttemptID getTaskAttemptID() {
    TaskID taskId = new TaskID(jobId, TaskType.MAP, this.taskId);
    return new TaskAttemptID(taskId, this.taskId++);
  }
