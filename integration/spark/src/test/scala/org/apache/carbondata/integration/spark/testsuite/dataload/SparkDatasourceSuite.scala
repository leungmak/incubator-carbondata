...
class SparkDatasourceSuite extends QueryTest with BeforeAndAfterAll {
  ...
  test("saveAsCarbonData API with compression") {
    import org.apache.carbondata.spark._
  ...
  test("use SQLContext to query") {
    ...
    sqlContext.sql(
      ...
      """.stripMargin)
    checkAnswer(sqlContext.sql("SELECT c1, c2, count(*) FROM source WHERE c3 > 100 GROUP BY c1, c2"),
      Seq(Row("a", "b", 900)))
