...
import org.apache.carbondata.scan.model.{CarbonQueryPlan, QueryDimension, QueryMeasure, QueryModel}
...
class CarbonScanRDDBuilder (
  ...
  def processFilterExpressions(plan: CarbonQueryPlan): Unit = {
  ...
  private def processExtraAttributes(plan: CarbonQueryPlan): Unit = {
    var output: Seq[Attribute] = attributesRaw
    ...
    output = dimAttr.filter(f => f != null) ++ msrAttr.filter(f => f != null)
  ...
  def build: RDD[Array[Any]] = {
    ...
    buildCarbonPlan.setQueryId(sqlContext.getConf("queryId", System.nanoTime() + ""))
