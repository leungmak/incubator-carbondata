...
public abstract class AbstractMeasureChunkReaderV2V3Format extends AbstractMeasureChunkReader {
   ...
   * @throws IOException
   */
  public MeasureRawColumnChunk[] readRawMeasureChunks(FileHolder fileReader, int[][] columnIndexRange)
      throws IOException {
    ...
    for (int i = 0; i < columnIndexRange.length - 1; i++) {
      index = 0;
      groupChunk = readRawMeasureChunksInGroup(fileReader, columnIndexRange[i][0], columnIndexRange[i][1]);
    ...
    if (columnIndexRange[columnIndexRange.length - 1][0] == measureColumnChunkOffsets.size() - 1) {
      ...
      groupChunk = readRawMeasureChunksInGroup(fileReader, columnIndexRange[columnIndexRange.length - 1][0],
