...
public class BlockletFullScanner implements BlockletScanner {
  ...
  @Override public BlockletScannedResult scanBlocklet(RawBlockletColumnChunks rawBlockletColumnChunks)
    ...
    for (int i = 0; i < measureRawColumnChunks.length; i++) {
      if (measureRawColumnChunks[i] != null) {
        columnPages[i] = measureRawColumnChunks[i].decodeAllColumnPages();
      }
    }
    scannedResult.setMeasureColumnPages(columnPages);
  ...
  @Override public void readBlocklet(RawBlockletColumnChunks rawBlockletColumnChunks) throws IOException {
    long startTime = System.currentTimeMillis();
