...
object GlobalDictionaryUtil {
  ...
  private def updateTableMetadata(carbonLoadModel: CarbonLoadModel,
    ...
    // update Metadata
    val carbonMetastore = CarbonEnv.get.carbonMetastore
    carbonMetastore.updateMetadataByThriftTable(schemaFilePath, tableInfo,
      model.table.getDatabaseName, model.table.getTableName, carbonLoadModel.getStorePath)

    // update CarbonDataLoadSchema
    val carbonTable = carbonMetastore.lookupRelation(Option(model.table.getDatabaseName),
      model.table.getTableName)(sqlContext.sparkSession).asInstanceOf[CarbonRelation].tableMeta.carbonTable
