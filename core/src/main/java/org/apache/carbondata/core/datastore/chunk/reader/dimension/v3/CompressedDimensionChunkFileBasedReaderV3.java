...
public class CompressedDimensionChunkFileBasedReaderV3 extends AbstractChunkReaderV2V3Format {
  ...
  @Override public DimensionColumnDataChunk convertToDimensionChunk(
    ...
    DataChunk2 pageHeader = dataChunk3.getData_chunk_list().get(pageNumber);
    ...
    return decodeDimension(rawColumnPage, rawData, pageHeader, offset);
  }

  private DimensionColumnDataChunk decodeDimensionByMeta(DataChunk2 pageHeader, ByteBuffer pageData, int offset)
      throws IOException, MemoryException {
    List<Encoding> encodings = pageHeader.getEncoders();
    List<ByteBuffer> encoderMetas = pageHeader.getEncoder_meta();
    ...
    switch (encoding) {
      ...
      case DIRECT_STRING:
        break;
      case GLOBAL_DICTIONARY_RLE_INTEGRAL:
        break;
      default:
        throw new UnsupportedOperationException("internal error");
    }
    ColumnPageDecoder decoder = strategy.createDecoder(encoding, metadata);
    ColumnPage decodedPage = decoder.decode(pageData.array(), offset, pageHeader.data_page_length);
    return new ColumnPageWrapper(decodedPage);
  }

  private boolean isEncodedWithNewWay(DataChunk2 pageHeader) {
    List<Encoding> encodings = pageHeader.getEncoders();
  ...
  private DimensionColumnDataChunk decodeDimension(DimensionRawColumnChunk rawColumnPage,
      ByteBuffer pageData, DataChunk2 pageHeader, int offset) throws IOException, MemoryException {
    if (isEncodedWithNewWay(pageHeader)) {
      return decodeDimensionByMeta(pageHeader, pageData, offset);
    }
    ...
    dataPage = COMPRESSOR.unCompressByte(pageData.array(), offset, pageHeader.data_page_length);
    offset += pageHeader.data_page_length;
    // if row id block is present then read the row id chunk and uncompress it
    if (hasEncoding(pageHeader.encoders, Encoding.INVERTED_INDEX)) {
      invertedIndexes = CarbonUtil
          .getUnCompressColumnIndex(pageHeader.rowid_page_length, pageData, offset);
      offset += pageHeader.rowid_page_length;
      // get the reverse index
      invertedIndexesReverse = getInvertedReverseIndex(invertedIndexes);
    }
    // if rle is applied then read the rle block chunk and then uncompress
    //then actual data based on rle block
    if (hasEncoding(pageHeader.encoders, Encoding.RLE)) {
      rlePage =
          CarbonUtil.getIntArray(pageData, offset, pageHeader.rle_page_length);
    ...
    if (!hasEncoding(pageHeader.encoders, Encoding.DICTIONARY)) {
      columnDataChunk =
          new VariableLengthDimensionDataChunk(dataPage, invertedIndexes, invertedIndexesReverse,
              pageHeader.getNumberOfRowsInpage());
    } else {
