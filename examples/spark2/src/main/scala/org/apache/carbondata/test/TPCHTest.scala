...
//    testParquet(spark)
  ...
  private def loadOrcString(sourceDataPath: String, spark: SparkSession): Unit = {
    println("loadOrcString")
    spark.sql("DROP TABLE IF EXISTS LINEITEM_orc_string")

    // Create table
    spark.sql(
      s"""
         | CREATE TABLE LINEITEM_orc_string (
    ...
    spark.sql(
      s"""
         | INSERT OVERWRITE TABLE LINEITEM_orc_string
         | SELECT L_SHIPINSTRUCT, L_SHIPMODE, L_COMMENT FROM LINEITEM_csv
       """.stripMargin)
  ...
  private def loadParquetString(sourceDataPath: String, spark: SparkSession): Unit = {
    println("loadParquetString")
    spark.sql("DROP TABLE IF EXISTS LINEITEM_parquet_string")

    // Create table
    spark.sql(
      s"""
         | CREATE TABLE LINEITEM_parquet_string (
         |	L_SHIPINSTRUCT	CHAR(25),
         |	L_SHIPMODE		CHAR(10),
         |	L_COMMENT		VARCHAR(44)
         | )
    ...
    spark.sql(
      s"""
         | INSERT OVERWRITE TABLE LINEITEM_parquet_string
         | SELECT L_SHIPINSTRUCT, L_SHIPMODE, L_COMMENT FROM LINEITEM_csv
       """.stripMargin)
  ...
  private def loadCarbonString(sourceDataPath: String, spark: SparkSession): Unit = {
    println("loadCarbonString")
    spark.sql("DROP TABLE IF EXISTS LINEITEM_carbon_string")

    // Create table
    spark.sql(
      s"""
         | CREATE TABLE LINEITEM_carbon_string (
         |	L_SHIPINSTRUCT	CHAR(25),
         |	L_SHIPMODE		CHAR(10),
         |	L_COMMENT		VARCHAR(44)
         | )
    ...
    spark.sql(
      s"""
         | LOAD DATA LOCAL INPATH '$sourceDataPath/lineitem'
         | INTO TABLE LINEITEM_carbon_string
         | OPTIONS ('delimiter'='|', 'header'='true', 'sort_scope'='no_sort')
       """.stripMargin)
