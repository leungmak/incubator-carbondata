...
object HarmonizeDimensionTable extends Rule[ModularPlan] with PredicateHelper {
  ...
  def apply(plan: ModularPlan): ModularPlan = plan transform {
    case s @ Select(_, _, _, _, jedges, fact :: dims, _, _, _) if jedges.forall(e => e.joinType == LeftOuter || e.joinType == Inner) &&
                                                                  fact.isInstanceOf[ModularRelation] && dims.filterNot(_.isInstanceOf[modular.LeafNode]).nonEmpty &&
...
object HarmonizeFactTable extends Rule[ModularPlan] with PredicateHelper with AggregatePushDown {
  ...
  def apply(plan: ModularPlan): ModularPlan = plan transform {
    case g @ GroupBy(_, _, _, _, s @ Select(_, _, _, aliasm, jedges, fact :: dims, _, _, _), _, _) if s.adjacencyList.keySet.size <= 1 &&
                                                                                                      ...
                                                                                                      fact.isInstanceOf[ModularRelation] && (fact :: dims).forall(_.isInstanceOf[modular.LeafNode]) && dims.nonEmpty => {
      ...
      if (aggTransMap.isEmpty ||
        ...
        val hFactSel = plans.modular
          .Select(sOutput, fact.output, Seq.empty, tAliasMap, Seq.empty, fact :: Nil, NoFlags, Seq.empty, Seq.empty)
        val hFact = plans.modular
          .GroupBy(gAttributes ++ oAggregates, sOutput, gAttributes, None, hFactSel, NoFlags, Seq.empty)
        val hFactName = s"gen_harmonized_${fact.asInstanceOf[ModularRelation].databaseName}_${fact.asInstanceOf[ModularRelation].tableName}"
