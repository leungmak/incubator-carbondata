...
public class CarbonS3FileSystem
    extends FileSystem
{
  private static LogService log = LogServiceFactory.getLogService(CarbonS3FileSystem.class.getCanonicalName());
  ...
  private static final long BLOCK_SIZE = 33554432;
  private static final long MAX_SKIP_SIZE = 1048576;
      ...
      throws IOException
  {
    ...
    this.stagingDirectory = new File(conf.get(S3_STAGING_DIRECTORY,
        defaults.getProperty(S3_STAGING_DIRECTORY)));
    ...
    boolean sslEnabled = conf.getBoolean(S3_SSL_ENABLED,
        Boolean.getBoolean(defaults.getProperty(S3_SSL_ENABLED)));
    ...
    this.sseEnabled = conf.getBoolean(S3_SSE_ENABLED,
        Boolean.getBoolean(defaults.getProperty(S3_SSE_ENABLED)));
    ...
    String userAgentPrefix = conf.get(S3_USER_AGENT_PREFIX,
        defaults.getProperty(S3_USER_AGENT_PREFIX));

    ClientConfiguration configuration = new ClientConfiguration()
        .withMaxErrorRetry(maxErrorRetries)
        .withProtocol(sslEnabled ? Protocol.HTTPS : Protocol.HTTP)
        .withConnectionTimeout(toIntExact(connectTimeout.getMillis()))
        .withSocketTimeout(toIntExact(socketTimeout.getMillis()))
      ...
      throws IOException
  {
    ...
    return new FSDataOutputStream(
        new CarbonS3OutputStream(s3, transferConfig, uri.getHost(), key, tempFile, sseEnabled, sseType, sseKmsKeyId),
        statistics);
  }
  ...
  private AmazonS3 createAmazonS3Client(URI uri, Configuration hadoopConfig, ClientConfiguration clientConfig)
  {
    ...
    if (emp.isPresent()) {
      client = AmazonS3EncryptionClientBuilder.standard()
          .withClientConfiguration(clientConfig)
          .withCredentials(credentials)
          .withCryptoConfiguration(new CryptoConfiguration())
          .withEncryptionMaterials(emp.get())
          .build();
    }
    else {
  ...
  public static class CarbonS3InputStream
      extends FSInputStream
  {
    ...
    public CarbonS3InputStream(AmazonS3 s3, String host, Path path, int maxAttempts, Duration maxBackoffTime, Duration maxRetryTime)
    {
  ...
  public static class CarbonS3OutputStream
      extends FilterOutputStream
  {
    ...
    {
      try {
        log.debug(String.format("Starting upload for host: %s, key: %s, file: %s, size: %s",
