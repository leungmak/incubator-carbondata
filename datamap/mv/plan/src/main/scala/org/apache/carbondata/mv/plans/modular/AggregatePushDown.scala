...
trait AggregatePushDown { // self: ModularPlan =>
  ...
  def findPushThroughAggregates(outputList: Seq[NamedExpression], selAliasMap: AttributeMap[Attribute], fact: ModularRelation): Map[Int, (NamedExpression,Seq[NamedExpression])] = {
    var pushable = true
...
//      case (alias: Alias, i) if (alias.child.isInstanceOf[Attribute] && fact.outputSet.contains(alias.child.asInstanceOf[Attribute])) => pushable = false
  ...
  private def transformAggregate(aggregate: AggregateExpression, selAliasMap: AttributeMap[Attribute], ith: Int, fact: ModularRelation, map: scala.collection.mutable.Map[Int, (NamedExpression, Seq[NamedExpression])], aliasInfo: Option[(String, ExprId)]) = {
    aggregate match {
